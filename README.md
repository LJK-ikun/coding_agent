...
前九章里,"这类活该怎么做"只有一个去处:`MEWCODE.md`——启动全读一遍,拼进 system。问题是它**只能常驻**:写测试的规矩、发版本的步骤、排查磁盘占满的套路,彼此无关,一段对话里顶多用到其中一条,却每条都要在**每一轮请求**里付一次钱。

ch10 把它们拆出来,做成**技能**。一个技能 = 一个 md 文件,拆成两半:

```markdown
---
name: offline-test
description: 给 MewCode 写测试时用。当你要新增/修改 tests/ 下的文件时用它。
---

（正文:这类活具体该怎么干……）
```

- **名片**(`name` + `description`)两行,启动时全部读出来,拼进 `use_skill` 工具的说明里,**一直挂着**
- **正文**可能几千字,**模型真要用时才读**:调一次 `use_skill(name="offline-test")`,正文作为一条工具结果进对话

一句话:**前三步模型只知道"有这么个技能",第四步才拿到"这技能具体怎么说"。**

清单放**工具的 description** 而不是 system,是因为工具清单本来就每轮随请求发(ch03 起就是这样)——放这儿就完全不用碰 ch05 的 system 装配与缓存断点,技能系统就是"一个工具"的事。

几条边界:

- 技能目录不存在 / 文件读不动 / 名片写坏了 → 一律**当没有**,不报错、不拦启动
- 一个技能都没有时**不注册** `use_skill`——否则模型手上多一个永远只能回"没有技能"的空工具
- 读目录时**排序**:技能清单是提示词的一段,顺序一乱前缀就变,ch05 的缓存全 miss
- 名字写错 → 失败收据里**带上候选**,模型下一次就填得对

技能放 `<工作目录>/.mewcode/skills/*.md`。

> 诚实的边界:技能**不隔离上下文**——它就是把一段说明加载进当前对话。要做到"派个独立 agent 去干、只收回结论、翻多少文件都不弄脏主历史",那是 SubAgent 的事,不是技能。
...
```bash
python -m pytest          # 272 条用例:LLM 传输层 17 + 工具系统 16 + ch04 Agent 3 + ch05 指令工程 14 + ch06 安全检查 59 + ch07 MCP 91 + ch08 上下文 24 + ch09 记忆 38 + ch10 技能 10
```
...
├── skills.py         # ch10:技能系统——md 拆名片+正文、use_skill 按需交回正文
└── tools/
...
- **ch09** 记忆:指令文件(`MEWCODE.md` + `@include`)+ 会话存档(JSONL 追加)+ 恢复(坏行跳过 / 悬空截断 / 压缩标记回放 / 久别提醒)+ 自动笔记(两级四类)
- **ch10** 技能(本版):把"这类活该怎么干"做成可插拔的 md,名片常驻、正文按需(`use_skill`)
- 后续规划:SubAgent(派独立 agent、隔离上下文)/ Team 编排、运行时指令注入、审计日志
