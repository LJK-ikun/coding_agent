...
# MewCode — tasks.md（ch10 实施顺序）

> 顺序 = 依赖拓扑。本章的特点：**先把"读"那半截做完、单测跑通，再接工具**。
> 因为解析有七八种边界（没目录、没名片、名片写坏、文件名兜底……），
> 这些跟"模型怎么用它"完全无关；混在一起调，出了错都不知道是哪半边的事。

## T1 技能文件 → Skill 对象 ✔
- **影响文件**：`mewcode/skills.py`（新建）
- **造**：`SKILLS_DIR`、`Skill`（frozen dataclass：name / description / body）、`_split(text)`（拆名片与正文）、`load_skills(base_dir)`
- **依赖**：无
- **参考**：土办法拆 frontmatter——第一行必须是 `---`，往下找第二个 `---`；找不到就**整篇当正文**，绝不猜。不用 yaml 库：名片只允许"键: 值"两行，土办法足够，省得引入解析失败 / 类型转换这些额外状况

## T2 按需加载的工具 ✔
- **影响文件**：`mewcode/skills.py`
- **造**：`UseSkillTool(skills)`——`name="use_skill"`，构造时把名片清单拼进 `description`（实例属性盖住类属性），`execute(name=...)` 返回正文
- **依赖**：T1
- **参考**：清单放 description 而不是 system，就完全不用碰 ch05 的装配与缓存断点；名字查不到时**报错要带候选**——只告诉模型"未知技能"，它只能瞎猜

## T3 接进主流程 ✔
- **影响文件**：`mewcode/cli.py`（改）
- **造**：`main()` 里 `load_skills(os.getcwd())` 传给 `run()`；`run()` 里 `if skills: registry.register(UseSkillTool(skills))`；启动横幅报技能个数；`run()` 的新参数给默认值
- **依赖**：T2
- **参考**：新参数默认 `None`——ch09 那些直接调 `run()` 的端到端测试一条都不用改。技能一个都没有时**不注册**工具，否则模型手上多一个永远只能回"没有技能"的空工具

## T4 测试与端到端验收 ✔
- **影响文件**：`tests/test_ch10_skills.py`（新建）
- **造**：10 条，三组——读文件 5（没目录 / 正常拆 / 文件名兜底 / 没名片 / 名片写坏）+ 按需加载 3（正常 / 未知名字 / 空名字）+ 名片清单 2（正文不进 description、schema 形状）
- **依赖**：T3
- **参考**：全部离线；每条自己造 tmp_path，在里头摆好 `.mewcode/skills/`。跑通 `python -m pytest` 全绿（旧 262 条不回归）

---

## 备注（ch10）
- 依赖链：T1 → T2 → T3 → T4。
- 本章**没动** ch03~ch09 的任何一行：技能就是一个普通工具，注册中心 / 执行器 / 门卫 / 缓存前缀，全都不用改。这是 ch03 定下 `Tool` 统一接口的好处。
- 备忘：仓库里还有一份 `mewcode/commands.py`（内置命令登记表，头注释也写着 ch10），**尚未接进 `cli.py`**，不属于本章范围。
